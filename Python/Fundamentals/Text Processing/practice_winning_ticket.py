...
def is_winner(_symbols, _ticket, _symbol_count):
    for symbol in _symbols:
        ...
        for combination in combinations:
            if combination in _ticket[:int(len(_ticket)/2)] and combination in _ticket[int(len(_ticket)/2):]:
...
def is_jackpot(_symbols, _ticket):
    for symbol in _symbols:
        if symbol * 10 in _ticket[:int(len(_ticket)/2)] and symbol * 10 in _ticket[int(len(_ticket)/2):]:
            return True
...
for ticket in tickets:
    if is_valid(ticket):
        if is_jackpot(symbols, ticket):
            print(f'ticket "{ticket}" - 10{Counter(ticket).most_common()[0][0]} Jackpot!')
        elif is_winner(symbols, ticket, symbol_count):
            print(f'ticket "{ticket}" - {symbol_count[0]}{Counter(ticket).most_common()[0][0]}')
            symbol_count.clear()
        else:
